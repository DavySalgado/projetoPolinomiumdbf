package com.DavySalgado.polinomium.web;
...
import com.DavySalgado.polinomium.domain.Licao;
import com.DavySalgado.polinomium.service.LicaoService;
...
public class LicaoResource {
    ...
    @PostMapping("/")
    public ResponseEntity<Licao> createPessoa(@RequestBody Licao user) throws URISyntaxException{
        Long id = user.getId();
        if(id != null){
    ...
    @PutMapping("/")
    public ResponseEntity<Licao> updateUsuario(@RequestBody Licao licao) throws URISyntaxException{
        if(licao.getId() == null){
